...
export const SettingsWindow: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      const language: LanguagePreference =
        settings.language === 'zh' || settings.language === 'en' || settings.language === 'system'
          ? settings.language
          : getLanguagePreference();
      setAutoStartup(as_);
  ...
  const handleAvatarRemove = useCallback(async () => {
    ...
    try {
      ...
      message.success(
        tl('头像已删除，已恢复名称首字符', 'Avatar removed; using the name initial again')
      );
    } catch (error) {
          ...
          >
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  '绿色仅作为强调色，界面背景与文本会根据亮色或暗色模式自动调整。',
                  'Green is used as an accent; surfaces and text adapt to light or dark mode.'
                )}
              </div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <span className="settings-card-title">
                  {tl('文件共享下载', 'File Sharing Downloads')}
                </span>
              </div>
              <div className="settings-card-desc">
                {tl(
                  '设置文件夹共享中下载的所有文件的保存位置，不影响聊天图片等其他下载。',
                  'Choose where all files downloaded from folder sharing are saved. Other downloads are not affected.'
                )}
              </div>
              <div className="settings-download-path-row">
                <Input
                  value={
                    ...
                    tl('系统默认下载目录（MCTier）', 'System default download folder (MCTier)')
                  }
                  readOnly
                ...
                <div className="settings-download-actions">
                  ...
                  {fileShareDownloadDir && (
                    <Button onClick={resetFileShareDownloadDir}>{tl('恢复默认', 'Reset')}</Button>
                  )}
                </div>
              </div>
            </motion.div>
                ...
                {autoLobbyEnabled && (
                  ...
                  >
                    <div className="settings-sub-form">
                      ...
                      >
                        {tl(
                          '说明：自动大厅会沿用与手动创建大厅相同的服务器。若已开启「使用私有服务器」，则使用你在私有服务器中配置的 EasyTier 节点与信令服务器；否则使用用户上次选择的节点（默认为海波美国节点）。',
                          'Note: Auto lobby uses the same server as manual lobby creation. If "Use private server" is enabled, it uses the configured EasyTier and signaling servers; otherwise it uses the last selected node (default: Haibo US Node).'
                        )}
                      </div>
                    </div>
                  </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <span className="settings-card-title">
                  {tl('用户共享节点', 'Community Shared Nodes')}
                </span>
              </div>
              <div className="settings-card-desc">
                {tl(
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <span className="settings-card-title">
                  {tl('麦克风权限', 'Microphone Permission')}
                </span>
              </div>
              <div className="settings-card-desc">
                {tl(
                  '首次拒绝后，若 WebView2 不再弹出授权窗口，可一键重置 MCTier 的权限缓存并自动重启。',
                  'If WebView2 no longer shows the permission prompt after access was denied, reset MCTier permission data and restart automatically.'
                )}
              </div>
              <div className="settings-centered-control microphone-reset-control">
                <Button
                  className="microphone-reset-button"
                  type="primary"
                  onClick={() => void invoke('reset_microphone_permission')}
                >
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-centered-control">
                <Button.Group className="mct-choice-group">
                  <Button
                    className="mct-choice-button"
                    type={lang === 'system' ? 'primary' : 'default'}
                    onClick={() => {
                      setLanguagePreference('system');
                      setLang('system');
                      void saveAll({ language: 'system' });
                    }}
                  >
                    {tl('自动', 'Auto')}
                  </Button>
...
const SoundThemeManager: React.FC = () => {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      antdMessage.success(
        tl(`已设置「${labels[target]}」自定义提示音`, `Custom sound set for "${labels[target]}"`)
      );
    };
...
const CustomNodeManager: React.FC = () => {
  ...
  if (loading) {
    return (
      <div style={{ padding: '16px', textAlign: 'center', color: 'rgba(255,255,255,0.6)' }}>
        {tl('加载中...', 'Loading...')}
      </div>
    );
  }
...
const CommunityNodeManager: React.FC = () => {
  ...
  const handleAdopt = async (node: CommunityNode) => {
    try {
      const cur = await invoke<any>('get_settings').catch(() => ({}) as any);
      const existing: Array<{ name: string; address: string }> = Array.isArray(
        cur?.customEasytierNodes
      )
  ...
  const renderStatus = (node: CommunityNode) => {
    if (node.online) {
      const latency =
        typeof node.latencyMs === 'number' ? `${node.latencyMs}ms` : tl('可用', 'online');
          ...
          >
            ...
            <Input
              placeholder={tl(
                ...
                'Node address (e.g. tcp://example.com:11010)'
              )}
              value={form.address}
              maxLength={128}
      ...
      {nodes.length === 0 ? (
        <div className="community-node-empty">
          {loading
            ...
            : tl(
                '暂无共享节点，欢迎投稿你的节点',
                'No shared nodes yet — feel free to submit yours'
              )}
        </div>
      ) : (
    ...
    <div className="config-manager">
      <div className="config-manager-buttons">
        ...
        <motion.button
          ...
          title={tl(
            '查看最近运行日志，方便定位问题',
            'View recent runtime logs for troubleshooting'
          )}
        >
          <svg
            width="16"
      ...
      <Modal
        ...
        width={820}
        footer={[
          <Button key="refresh" onClick={() => void loadLogs()} loading={loadingLogs}>
            {tl('刷新', 'Refresh')}
          </Button>,
          <Button key="copy" onClick={() => void handleCopyLogs()} disabled={!logs}>
            {tl('复制日志', 'Copy Logs')}
          </Button>,
          <Button
            key="export"
