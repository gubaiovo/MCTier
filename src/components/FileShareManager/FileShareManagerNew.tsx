...
function safeDownloadFileName(fileName: string): string {
  ...
  let safe = basename.replace(/[<>:"/\\|?*\u0000-\u001f\u007f]/g, '_').replace(/[. ]+$/g, '');
  if (!safe || safe === '.' || safe === '..') {
...
function normalizeSharedFolderSummary(value: unknown): SharedFolderSummary | null {
  ...
  const expireTime =
    typeof item.expire_time === 'number' && Number.isFinite(item.expire_time)
...
function normalizeFileInfo(value: unknown): FileInfo | null {
  ...
  const size =
    typeof item.size === 'number' && Number.isSafeInteger(item.size)
      ...
      : -1;
  const modified =
    typeof item.modified === 'number' && Number.isFinite(item.modified)
...
function buildDownloadUrl(ownerIp: string, shareId: string, filePath: string): string | null {
  if (
    !isSafeVirtualIp(ownerIp) ||
    !isSafeResourceId(shareId) ||
...
function parseDownloadUrl(
  url: string
): { peerIp: string; shareId: string; filePath: string } | null {
  try {
    const parsed = new URL(url);
    if (parsed.protocol !== 'http:' || parsed.port !== '14539' || !isSafeVirtualIp(parsed.hostname))
      return null;
...
export const FileShareManagerNew: React.FC = () => {
  ...
  const requestDownloadPassword = (remoteShare: SimpleRemoteShare, retry: DownloadRetry) => {
    ...
    message.error(
      tl('下载需要密码，请输入后重试', 'Download requires a password; enter it to retry')
    );
  };
  ...
  const forgetRemoteShare = (remoteShare: SimpleRemoteShare, expired: boolean) => {
    setRemoteShares((prev) =>
      prev.filter(
        (s) => !(s.ownerIp === remoteShare.ownerIp && s.share.id === remoteShare.share.id)
      )
    );
  ...
  const loadFiles = async (
    ...
    try {
      ...
      try {
        ...
        if (
          detail.includes('401') ||
          detail.includes('密码错误') ||
          detail.includes('访问被拒绝')
        ) {
      ...
      if (isUnauthorizedDownloadError(error) && selectedShare) {
          ...
          prev.map((task) =>
            task.id === taskId
              ? {
                  ...
                  error: tl('等待重新输入密码', 'Waiting for password'),
                  speed: 0,
                }
              : task
          )
        ...
        requestDownloadPassword(selectedShare, (password) => {
          ...
          setDownloads((prev) =>
            prev.map((task) =>
              task.id === taskId
                ? { ...task, status: 'downloading' as const, error: undefined, downloaded: 0 }
                : task
            )
          );
  ...
  const handleBatchDownload = async () => {
    ...
    const selectedFileList = files.filter(
      (f) => !f.is_dir && selectedFiles.has(f.path) && isSafeRelativePath(f.path)
    );
    ...
    if (selectedShare.share.compress_before_send && selectedFileList.length > 1) {
      try {
        ...
        const tempZipPath = await invoke<string>('get_file_share_download_path', {
        ...
        (async () => {
          ...
          try {
            ...
            const filePaths = selectedFileList
              .map((f) => f.path)
            ...
            if (
              filePaths.length !== selectedFileList.length ||
              !isSafeVirtualIp(selectedShare.ownerIp) ||
              !isSafeResourceId(selectedShare.share.id)
            ) {
            ...
            const extractedFiles = await invoke<string[]>('extract_zip', {
              zipPath: tempZipPath,
              extractDir: saveDir,
              maxTotalBytes: selectedFileList.reduce(
                (total, item) => total + Math.max(0, item.size),
                0
              ),
            });
              ...
              prev.map((task) =>
                task.id === taskId
                  ? {
                      ...
                      speed: 0,
                      fileName: tl(
                        `${selectedFileList.length} 个文件`,
                        `${selectedFileList.length} files`
                      ), // 更新显示名称
      ...
      for (const file of selectedFileList) {
        const safeName = safeDownloadFileName(file.name);
        const savePath = await invoke<string>('get_file_share_download_path', {
          fileName: safeName,
        });
        const downloadUrl = buildDownloadUrl(
          selectedShare.ownerIp,
  ...
  return (
    <div className="file-share-container">
            ...
            {activeTab === 'local' && (
              ...
              >
                <div className="share-list">
                  <Button
                    className="file-share-primary-btn"
                    type="primary"
                      ...
                      >
                        ...
                        <div className="share-info">
                          ...
                          <div className="share-meta">
                            {share.password && '🔒 '}
                            {share.compress_before_send && '📦 '}
                            {share.expire_time && `⏰ ${formatTime(share.expire_time)}`}
                          </div>
                        </div>
                        <button
      ...
      <Modal
        className="file-share-modal"
        rootClassName="file-share-modal-root"
