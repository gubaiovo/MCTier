...
export const ExitNodeAdvancedModal: React.FC<ExitNodeAdvancedModalProps> = ({
  ...
  // 更新端口转发规则
  const handleUpdatePortForwardRule = (
    index: number,
    field: keyof PortForwardRule,
    value: string
  ) => {
    ...
    >
      <div className="exit-node-advanced-content">
        <div className="exit-node-advanced-header">
          ...
          <p>
            {tl(
              '配置 SOCKS5 代理、端口转发和其他高级功能',
              'Configure SOCKS5 proxy, port forwarding and other advanced features'
            )}
          </p>
        </div>

        <Spin spinning={loading}>
          <Form form={form} layout="vertical" className="exit-node-advanced-form">
            {/* SOCKS5 代理 */}
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  <span className="toggle-label">
                    {tl('启用 SOCKS5 代理', 'Enable SOCKS5 Proxy')}
                  </span>
                  <span className="toggle-desc">
                    {tl(
                      '开启后可以通过 SOCKS5 协议访问虚拟网络',
                      'When enabled, access the virtual network via the SOCKS5 protocol'
                    )}
                  </span>
                </div>
                <Switch
                  ...
                  >
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: true,
                          message: tl('请输入端口号', 'Please enter a port number'),
                        },
                        {
                          type: 'number',
                          min: 1024,
            ...
            <div className="config-section">
              ...
              <p className="section-desc">
                {tl(
                  ...
                  "Forward a local port to a remote port in the virtual network. e.g. forward the exit node's SOCKS5 port locally."
                )}
              </p>
                ...
                {portForwardRules.map((rule, index) => (
                  <div key={index} className="port-forward-rule">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleUpdatePortForwardRule(index, 'bind_addr', e.target.value)
                      }
                      placeholder={tl(
                        '本地地址（例如：0.0.0.0:5678）',
                        'Local address (e.g. 0.0.0.0:5678)'
                      )}
                      style={{ flex: 1 }}
                    />
                    ...
                    <Input
                      value={rule.dst_addr}
                      onChange={(e) =>
                        handleUpdatePortForwardRule(index, 'dst_addr', e.target.value)
                      }
                      placeholder={tl(
                        '目标地址（例如：10.2.2.1:5678）',
                        'Target address (e.g. 10.2.2.1:5678)'
                      )}
                      style={{ flex: 1 }}
                    />
                    <button
              ...
              <button className="add-rule-btn" onClick={handleAddPortForwardRule}>
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
            ...
            <div className="config-section">
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl(
                      '不创建虚拟网卡，仅通过子网代理访问节点',
                      'Do not create a virtual adapter; access nodes only via subnet proxy'
                    )}
                  </span>
                </div>
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  <span className="toggle-label">
                    {tl('仅使用物理网卡', 'Use Physical Adapter Only')}
                  </span>
                  <span className="toggle-desc">
                    {tl(
                      '绑定物理设备避免路由问题',
                      'Bind the physical device to avoid routing issues'
                    )}
                  </span>
                </div>
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl(
                      '使用多线程运行时提升性能',
                      'Use a multithreaded runtime to improve performance'
                    )}
                  </span>
                </div>
                      ...
                      rules={[
                        {
                          type: 'number',
                          min: 2,
                          message: tl('线程数量必须大于等于 2', 'Thread count must be at least 2'),
                        },
                      ]}
                    >
                      <InputNumber
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl(
                      '为子网代理和 KCP 代理启用 smoltcp 堆栈',
                      'Enable the smoltcp stack for subnet proxy and KCP proxy'
                    )}
                  </span>
                </div>
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl(
                      '使用 KCP 代理 TCP 流，提高在 UDP 丢包网络上的性能',
                      'Proxy TCP streams over KCP to improve performance on lossy UDP networks'
                    )}
                  </span>
                </div>
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl(
                      '使用 QUIC 代理 TCP 流，提高在 UDP 丢包网络上的性能',
                      'Proxy TCP streams over QUIC to improve performance on lossy UDP networks'
                    )}
                  </span>
                </div>
              ...
              <div className="config-toggle">
                <div className="toggle-info">
                  ...
                  <span className="toggle-desc">
                    {tl('使用最低延迟路径转发流量', 'Forward traffic over the lowest-latency path')}
                  </span>
                </div>
